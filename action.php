...
<html>
    ...
    <?php
        ...
        if (isset($_POST['submit1'])){
                ...
                $namabaru = "foto/".$_POST['username']."-".date('YmdHis').".".$extension;
        ...
        include 'dbconfig.php';
            ...
            $query = "INSERT INTO score (username, playtime, foto) VALUES ('".$_COOKIE['username']."' , '".date('Y-m-d H:i:s')."','".$_COOKIE['foto']."')";
    ...
    ?>
        ...
        <div style="-moz-user-select: none; -webkit-user-select:none; -ms-user-select:none; user-select:none; -moz-user-drag:none; -webkit-user-drag:none; -ms-user-drag:none; user-drag: none;" unselectable="on">
            <?php  echo "<h2>Welcome ".$_COOKIE['username'].". </h2>"; ?>
            <?php
...
class Game {
    ...
    function game_check() {
        ...
        if ($this->grid_size % 2 == 0 || $this->grid_size < 3 ) {
    ...
    function show_cell($which) {
        ...
        if ($token <> '-') {
            ...
            return '<td style="text-align:center;' . ($token != $player_board[$which] ? ' background-color: #FFA500;' : '' ) . '"><div style="padding: 2em;">' . $token . ($this->debug ? ('<br /><span style="font-size:100%;">' . $which . ':(' . ((int) ($which / $this->grid_size) + 1) . ',' . (($which % $this->grid_size) + 1) . ')</span>') : '') . '</div></td>';
        }
        // now the hard case
        ...
        return '<td style="text-align:center;"><a href = "' . $link . '" style = "text-decoration: none;"><div style="padding: 2em;">-' . ($this->debug ? ('<br /><span style="font-size:100%;">' . $which . ':(' . ((int) ($which / $this->grid_size) + 1) . ',' . (($which % $this->grid_size) + 1) . ')</span>') : '') . '</div></a></td>';
    }
    ...
    function game_message($message) {
        ...
        switch ($message) {
            ...
            case 'invalid-character':
                ...
                break;
            case 'x-win':
                echo '<strong><h2>Selamat! Anda Memenangkan Game Ini.</h2></strong>';
                break;
            case 'o-win':
                echo '<strong><h2>Yahh :( Anda Kalah. Tetap Semangat. Ulangi lagi, Jangan Menyerah :)</h2></strong>';
                break;
            case 'tie-game':
                echo '<strong><h2>Anda Bermain Imbang. Ulangi Lagi!</h2></strong>';
        ...
        if ($newGame) {
            ...
            echo '<br /><br /><a draggable="false" href="' . $_SERVER['PHP_SELF'] . '?size=' . $this->grid_size . '" style="-webkit-appearance: button; -moz-appearance: button; appearance: button; text-decoration: none; color: initial; padding: 0.5em;">Mulai Ulang' . ($this->debug ? ' (no debug info)' : '') . '!</a>' ;
            echo '<br /><br /><a draggable="false" href=" home.php' . '?size=' . $this->grid_size . '" style="-webkit-appearance: button; -moz-appearance: button; appearance: button; text-decoration: none; color: initial; padding: 0.5em;">Keluar' . ($this->debug ? ' (no debug info)' : '') . '!</a>' ;
